...
void Add() //Функция добавления встречи
{
    ...
    do
    {
        ...
        Clear();
        WriteLine("Введена некорректная дата, попробуйте еще раз! (Возможно, вы ввели конец встречи который случиться раньше чем начало встречи)");
    } while (true);
    meet.Add(
        new Vstrecha()
        {
...
DateTime DateIn() //Функция ввода даты/времени
{
